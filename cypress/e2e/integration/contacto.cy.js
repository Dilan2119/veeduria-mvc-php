describe('Prueba la autenticación', () => {
    ...
    cy.get("form.formulario").submit();
        ...
        cy.visit('http://localhost:3000/contactos/administrarContacto'); 
// Verificar la existencia de los elementos principales
cy.get('h2').should('contain', 'Contactos');
cy.get('a.boton-verde').should('have.attr', 'href', '/contactos/crear');
cy.get('a.boton-amarillo').should('have.attr', 'href', '/admin');
cy.get('table.propiedades').should('exist');

// Verificar la tabla de contactos
...
cy.get('table.propiedades tbody tr').each(($tr, index) => {
  ...
  cy.wrap($tr).find('td').eq(1).should('exist'); // Verificar la existencia de la columna "Nombre"
  cy.wrap($tr).find('td').eq(2).should('exist'); // Verificar la existencia de la columna "Teléfono"

  // Verificar los botones de acción
  cy.wrap($tr).find('td').eq(3).find('form').should('exist');
  cy.wrap($tr).find('td').eq(3).find('form input[type="submit"]').should('have.value', 'Eliminar');
  cy.wrap($tr).find('td').eq(3).find('a').should('exist');
  cy.wrap($tr).find('td').eq(3).find('a').should('have.attr', 'href').and('include', '/contactos/actualizar');
});
});
}) 